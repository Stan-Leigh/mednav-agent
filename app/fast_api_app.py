...
try:
    ...
    # Add mock log_struct for compatibility
    logger.log_struct = lambda data, severity="INFO": logger.info(f"[{severity}] {data}")
allow_origins = (
...
app.title = "mednav-agent"
app.description = "API for interacting with the Agent mednav-agent"
...
# Remove default root route to override with custom playground redirect
app.routes[:] = [r for r in app.routes if r.path != "/"]


@app.get("/")
def redirect_to_custom_playground():
    from fastapi.responses import RedirectResponse
    return RedirectResponse(url="/playground")


@app.get("/playground")
def get_custom_playground():
    from fastapi.responses import HTMLResponse
    import os
    current_dir = os.path.dirname(os.path.abspath(__file__))
    filepath = os.path.join(current_dir, "playground.html")
